Функция ВыполнитьПриложение(Ссылка, ТипПодключаемойОбработки) Экспорт
	...
	Запрос.Текст =
		...
		|	ТН.Ссылка.АЭМ_ТипПеревозки КАК АЭМ_ТипПеревозки,
		|	ТН.Ссылка.Организация.НаименованиеПолное КАК ОрганизацияНаименование,
		|	ТН.Ссылка.Организация.ИНН КАК ОрганизацияИНН,
		|	ТН.Ссылка.Организация.КПП КАК ОрганизацияКПП,
		|	ТН.Ссылка.Грузоотправитель КАК Грузоотправитель,
		|	ТН.Ссылка.Грузоотправитель.НаименованиеПолное КАК ГрузоотправительНаименование,
		|	ТН.Ссылка.Грузоотправитель.ИНН КАК ГрузоотправительИНН,
		|	ТН.Ссылка.Грузоотправитель.КПП КАК ГрузоотправительКПП,
		|	ТН.Ссылка.Грузополучатель.Партнер.Код КАК ГрузополучательПартнерКод,
		|	ТН.Ссылка.Грузополучатель.НаименованиеПолное КАК ГрузополучательНаименование,
		|	ТН.Ссылка.Грузополучатель.ИНН КАК ГрузополучательИНН,
		|	ТН.Ссылка.Грузополучатель.КПП КАК ГрузополучательКПП,
		|	ТН.Ссылка.Плательщик.Партнер.Код КАК ПлательщикПартнерКод,
		|	ТН.Ссылка.Плательщик КАК Плательщик,
		|	ТН.Ссылка.Плательщик.НаименованиеПолное КАК ПлательщикНаименование,
		|	ТН.Ссылка.Плательщик.ИНН КАК ПлательщикИНН,
		|	ТН.Ссылка.Плательщик.КПП КАК ПлательщикКПП,
		|	ТН.Ссылка.Перевозчик.НаименованиеПолное КАК ПеревозчикНаименование,
		|	ТН.Ссылка.Перевозчик.ИНН КАК ПеревозчикИНН,
		|	ТН.Ссылка.Перевозчик.КПП КАК ПеревозчикКПП,
		|	ТН.Ссылка.АЭМ_СтанцияИДорогаНазначения КАК АЭМ_СтанцияИДорогаНазначения,
		|	ТН.Ссылка.АЭМ_Ветка КАК АЭМ_Ветка,
		|	ТН.Ссылка.АЭМ_НомерЖдВагона КАК АЭМ_НомерЖдВагона,
		|	ТН.Ссылка.АЭМ_НомерЖдНакладной КАК АЭМ_НомерЖдНакладной,
		|	ТН.Ссылка.АЭМ_НомерСдаточнойНакладной КАК АЭМ_НомерСдаточнойНакладной,
		|	ТН.Ссылка.АвтомобильГосударственныйНомер КАК АвтомобильГосударственныйНомер,
		|	ТН.Ссылка.Водитель КАК Водитель,
		|	ТН.Ссылка.АвтомобильМарка КАК АвтомобильМарка,
		|	ТН.Ссылка.АвтомобильТип КАК АвтомобильТип,
		|	ТН.ДокументОснование КАК ДокументОснование,
		|	ТН.Ссылка.ГосударственныйНомерПрицепа КАК Прицеп
		|ИЗ
		|	Документ.ТранспортнаяНакладная.ДокументыОснования КАК ТН
		|ГДЕ
		|	ТН.Ссылка = &Ссылка";
...
Функция ВыгрузитьВСАКВТ(Выборка, Шапка, Грузы, ТипПодключаемойОбработки, Отказ = Ложь)
	...
	СтруктураДанных 	= АЭМ_ШтрихкодированиеПечатныхФорм.ДанныеДляПечатиШтрихКодаТТН(
									Выборка.Ссылка, Выборка.Грузоотправитель, Выборка.Плательщик);
	document.uid		= АЭМ_ШтрихкодированиеПечатныхФорм.ПолучитьШтрихКод(СтруктураДанных);
	document.rows     	= rows;
	...
	Если Выборка.АЭМ_ТипПеревозки = Перечисления.АЭМ_ТипыПеревозок.Авто Тогда
		document.type      = "01"; // ТТН на вывоз ТМЦ
	ИначеЕсли Выборка.АЭМ_ТипПеревозки = Перечисления.АЭМ_ТипыПеревозок.Жд Тогда
		document.type      = "16"; // ТТН на вывоз ТМЦ
	КонецЕсли;
