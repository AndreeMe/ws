Функция ВыполнитьПриложение(РаспоряжениеНаПровес, ТипПодключаемойОбработки) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Сообщение.Текст = "Не задано описание веб-сервиса для сообщений вида " + ТипПодключаемойОбработки;
		Сообщение.Сообщить(); 
		Возврат Ложь;		
	КонецЕсли; 
...
//		|	АЭМ_РаспоряжениеНаПровес.ТипОперации КАК OPERATION,
	...
	Для каждого Кол Из РезультатЗапроса.Колонки Цикл
		...
		КонецЕсли; 
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи[Кол.Имя]) Тогда
			item[Кол.Имя] = XMLСтрока(ВыборкаДетальныеЗаписи[Кол.Имя]);	
		Иначе
