...
Функция СформироватьОтветноеСообщение(ОбXDTO)
	...
	СтруктураДанных = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ОбXDTO);
	 ...
	 Если СтруктураДанных.Свойство("op") И НЕ СтруктураДанных.op.Количество() = 0 Тогда
		// если есть "ор", то это термообработка
	 	Возврат СформироватьОтветноеСообщениеТермоорбработка(ОбXDTO);
	 КонецЕсли; 
	 
	 Если СтруктураДанных.Свойство("item") И НЕ СтруктураДанных.item.Количество() = 0  Тогда
...
Функция ОбработатьМассивСтруктурТермоорбработка(Об)
	...
	ДлинаНомера = Метаданные.Документы.ЗаказНаПроизводство2_2.ДлинаНомера;
	Шаблон = "                     ";
	...
	Если ТипЗнч(Об) = Тип("ОбъектXDTO") Тогда
		...
		Если ТипЗнч(Эл.KDPOS) = Тип("Строка") Тогда
			Отбор.KDPOS = СокрЛП(Об.KDPOS) + Лев(Шаблон, ДлинаНомера - СтрДлина(Об.KDPOS));
			Выборка.Сбросить();
	...
	ИначеЕсли ТипЗнч(Об) = Тип("СписокXDTO") Тогда
		Для каждого Эл Из Об Цикл
			Если НЕ ТипЗнч(Эл.KDPOS) = Тип("Строка") Тогда
				Продолжить;
			КонецЕсли; 
			// Эл.LTXA1 = ПерекодироватьСтроку(Эл.LTXA1, КодировкаТекста.UTF8);
			Отбор.KDPOS = СокрЛП(Эл.KDPOS) + Лев(Шаблон, ДлинаНомера - СтрДлина(Эл.KDPOS));
			Выборка.Сбросить();
			Если Выборка.НайтиСледующий(Отбор) Тогда
...
Функция ОбработатьМассивСтруктурКовка(Об)
	...
	Пока Выборка.Следующий() Цикл
		...
		// + нумеруем позиции в пределах заказов (А.С. 10.03.21) 
		Если НЕ KDAUF = СокрЛП(Выборка.KDAUF) Тогда
		   VTPOS = 0;
		   KDAUF = СокрЛП(Выборка.KDAUF);
		КонецЕсли; 
		VTPOS = VTPOS + 1;
		// -
		
		item = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://aem_intermech", "item"));  
		item.KDPOS = XMLСтрока(СокрЛП(Выборка.KDPOS));
		item.KDAUF = XMLСтрока(СокрЛП(Выборка.KDAUF));
		item.VTPOS = XMLСтрока(VTPOS);
		item.LTXA1 = XMLСтрока(Выборка.LTXA1);
		item.ARBPL = XMLСтрока(Выборка.ARBPL);
		item.CEH   = XMLСтрока(Выборка.CEH);
		// В справочнике «Циклы ПДУ» и всех с ним связанных сроки хранятся в днях.
		// переводим дни в часы...
		item.VGW01 = 24 * ОписаниеТипаЧисло.ПривестиЗначение(Выборка.VGW01); 
...
Функция ДлительностьТермообработкиПоЗаказам(МассивНомеров)
	...
	//// из-за необхомости удаления пробелов из номера...
	//Таб = РезультатЗапроса.Выгрузить();
	//Таб.Колонки.KDPOS.ТипЗначения.КвалификаторыСтроки.Длина = 0;
	//Для каждого Стр Из Таб Цикл
	//	Стр.KDPOS = СокрЛП(Стр.KDPOS);
	//КонецЦикла; 
...
Функция ДлительностьОперацийКовкиПоЗаказам(МассивНомеров, Период = Неопределено)
	...
	Запрос.Текст = 
		...
		|			И ЗаказНаПроизвПродукция.Ссылка.АЭМ_ИдентификаторСтрокиКАЗ = ЗаказКлиентаТовары.АЭМ_ИдентификаторСтрокиКАЗ
		|ГДЕ
